...
def init_db():
    ...
    """)
    # 奖励收集表（主键为 user_id+chat_id，支持同一人多个群获奖）
    conn.execute("""
    ...
    """)
    # 兼容旧表迁移：旧表用 user_id 单主键，重建为复合主键 (user_id, chat_id)
    try:
        cols = [row[1] for row in conn.execute('PRAGMA table_info(award_submissions)').fetchall()]
        pk_cols = [row[1] for row in conn.execute('PRAGMA table_info(award_submissions)').fetchall() if row[5] > 0]
        needs_migrate = 'merch_address' not in cols or len(pk_cols) < 2
        if needs_migrate:
            conn.execute('ALTER TABLE award_submissions RENAME TO award_submissions_old')
            conn.execute("""
            ...
            """)
            old_cols2 = [row[1] for row in conn.execute('PRAGMA table_info(award_submissions_old)').fetchall()]
            merch_col = 'merch_address' if 'merch_address' in old_cols2 else "''"
            chat_id_col = 'chat_id' if 'chat_id' in old_cols2 else "''"
            conn.execute(f"""
                INSERT OR IGNORE INTO award_submissions
                (user_id, chat_id, username, full_name, chat_title, season_label,
                 rank, reward_usdt, address_type, address, binance_uid, merch_address, submitted_at)
                SELECT user_id, {chat_id_col}, username, full_name, chat_title, season_label,
                       rank, reward_usdt, address_type, address, binance_uid, {merch_col}, submitted_at
                FROM award_submissions_old
            """)
            conn.execute('DROP TABLE award_submissions_old')
            conn.commit()
    except Exception as e:
        logger.warning(f'[db] award_submissions migration warning: {e}')
    # 奖励通知已发消息记录（用于撤回）
    conn.execute("""
...
async def _binance_listing_poll_job(app):
    ...
    try:
        r = subprocess.run(
            ["curl", "-s", "--proxy", _PROXY, "--max-time", "10",
             "-H", "lang: zh-CN",
...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # 漏洞一修复：排行榜只统计真实人数 >= 50 的群，防空壳群刷榜
        # 使用内存缓存 _member_count_cache，避免实时查询造成超时
        all_chats_raw = conn.execute("""
            SELECT chat_id, chat_title, COUNT(*) as score
            FROM queries
        ...
        # 用缓存人数过滤，缓存没有的群实时查一次（单个查询，加 5s 超时）
        chats = []
        for cid, title, score in all_chats_raw:
            if len(chats) >= 10:
                break
            cached = _member_count_cache.get(str(cid))
            if cached:
                count = cached[1]
            else:
                try:
                    count = await asyncio.wait_for(
                        context.bot.get_chat_member_count(chat_id=int(cid)),
                        timeout=5.0
                    )
                    _member_count_cache[str(cid)] = (asyncio.get_event_loop().time(), count)
                except Exception:
                    count = 0  # 查不到的群一律排除
            if count >= 50:
                chats.append((title, score))

        rank_lines = []
        for i, (title, score) in enumerate(chats):
            icon = medals[i] if i < len(medals) else f"{i+1}."
            rank_lines.append(f"{icon} {he(censor_title(title))}  <b>{score}</b>分")
        ...
        await update.message.reply_text("统计查询失败，请稍后再试。")
...
AWARD_SEASON_LABEL = "第三赛季"
AWARD_RANKINGS = [
    {"rank": 1,  "chat_id": -1002797443448, "chat_title": "MemeticMonk狗宝殿",      "reward_usdt": 100},
    {"rank": 2,  "chat_id": -1003873218217, "chat_title": "曼波猪脚饭",              "reward_usdt": 30},
    {"rank": 3,  "chat_id": -1004389571385, "chat_title": "只要肝不死就往死里干",    "reward_usdt": 30},
    {"rank": 4,  "chat_id": -1002469262099, "chat_title": "专心搞U，偶尔悟道",       "reward_usdt": 20},
    {"rank": 5,  "chat_id": -1003874401650, "chat_title": "八方社区——Predict冠名",  "reward_usdt": 20},
    {"rank": 6,  "chat_id": -1003061233409, "chat_title": "天天返佣网",              "reward_usdt": 20},
    {"rank": 7,  "chat_id": -1003943755676, "chat_title": "三建学园（方外之境）",    "reward_usdt": 20},
    {"rank": 8,  "chat_id": -1002884255646, "chat_title": "币小超社区",              "reward_usdt": 20},
    {"rank": 9,  "chat_id": -1002357030194, "chat_title": "比特币炒币交流群",        "reward_usdt": 20},
    {"rank": 10, "chat_id": -1001558317950, "chat_title": "芳心未艾",               "reward_usdt": 20},
]
AWARD_LUCKY = [
    {"chat_id": -1003735344060, "chat_title": "小董和她的富豪朋友们"},
    {"chat_id": -1003536878833, "chat_title": "ZDS Club"},
    {"chat_id": -1002958321032, "chat_title": "鱼泡泡撸毛"},
]
RANK_ICONS = {1: "🥇", 2: "🥈", 3: "🥉"}
AWARD_POSTER_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "award_poster_s3.jpg")
...
async def _find_winner_rank(bot, user_id: int):
    """查找用户是哪个排名群的群主（只返回第一个，兼容旧调用）"""
    results = await _find_all_winner_ranks(bot, user_id)
    return results[0] if results else None


async def _find_all_winner_ranks(bot, user_id: int):
    """查找用户在所有排名群中的群主身份，支持一人多群，返回列表。
...
async def _find_lucky_winner(bot, user_id: int):
    """查找用户是否是幸运社区的群主，返回 lucky dict 或 None"""
    for lucky in AWARD_LUCKY:
        try:
            ...
            try:
                member = await bot.get_chat_member(lucky["chat_id"], user_id)
                if member.status == "creator":
                    return lucky
            except Exception:
                continue
...
async def _check_and_greet_award_winner(update, context) -> bool:
    """私聊 /start 时检查是否为获奖群主（含幸运社区），是则发奖励通知+选择按钮。
    支持一人多个获奖群，每个群单独发一条通知。返回 True 表示已处理。"""
    user = update.effective_user
    if not user:
        return False
    ...
    # 检查所有排名群（支持一人多群）
    rank_list = await _find_all_winner_ranks(context.bot, user.id)
    if rank_list:
        try:
        ...
        for rank_info in rank_list:
            ...
            cid   = rank_info["chat_id"]

            if rank <= 3:
                kb = InlineKeyboardMarkup([[
                    InlineKeyboardButton(f"💰 领取 {usdt} USDT", callback_data=f"award:usdt:{rank}:{cid}"),
                ]])
                choice_hint = f"奖励：<b>{usdt} USDT</b>"
            else:
                kb = InlineKeyboardMarkup([[
                    InlineKeyboardButton(f"💰 领取 {usdt} USDT", callback_data=f"award:usdt:{rank}:{cid}"),
                    InlineKeyboardButton("🎒 币安双肩包", callback_data=f"award:merch:{rank}:{cid}"),
                ]])
    ...
    # 检查幸运社区
    lucky_info = await _find_lucky_winner(context.bot, user.id)
    if lucky_info:
        title = lucky_info["chat_title"]
        cid   = lucky_info["chat_id"]
        kb = InlineKeyboardMarkup([[
            InlineKeyboardButton("🎒 填写收货信息", callback_data=f"award:merch:lucky:{cid}"),
        ]])
        if not handled:  # 海报只发一次
            try:
                with open(AWARD_POSTER_PATH, "rb") as f:
                    await update.message.reply_photo(photo=f)
            except Exception as e:
                logger.warning(f"[award] poster send failed: {e}")
        await update.message.reply_text(
            f"🎁 <b>幸运社区奖励通知 — {AWARD_SEASON_LABEL}</b>\n\n"
            f"恭喜你的群 <b>{title}</b> 被选为本赛季幸运社区！\n"
            f"奖励：🎒 <b>币安品牌双肩包 ×1</b>\n\n"
            f"━━━━━━━━━━━━━━━━\n"
            f"请点击下方按钮填写收货信息 👇",
            parse_mode="HTML",
...
async def _handle_award_callback(update, context) -> bool:
    ...
    callback_data 格式: award:<action>:<rank_or_lucky>:<chat_id>
    """
    query = update.callback_query
    ...
    action   = parts[1]
    rank_raw = parts[2] if len(parts) > 2 else ""
    ...
    # 幸运社区 — 只有实物奖励
    if rank_raw == "lucky":
        # 优先用 callback_data 里的 chat_id 定位正确的幸运社区
        lucky_info = None
        if cid_raw:
            lucky_info = next((l for l in AWARD_LUCKY if str(l["chat_id"]) == cid_raw), None)
        if not lucky_info:
            lucky_info = await _find_lucky_winner(context.bot, user.id)
        title = lucky_info["chat_title"] if lucky_info else "你的群"
        lucky_chat_id = str(lucky_info["chat_id"]) if lucky_info else cid_raw or "0"
        fake_rank_info = {"rank": 0, "chat_id": lucky_chat_id, "chat_title": title, "reward_usdt": 0}
        pending_key = f"{user_id}:{lucky_chat_id}"
        _merch_pending[pending_key] = {"rank_info": fake_rank_info, "step": "waiting_address", "is_lucky": True}
    ...
    rank = int(rank_raw)
    # 优先用 callback_data 里的 chat_id 定位排名群，避免多群时取错
    if cid_raw:
        rank_info = next((r for r in AWARD_RANKINGS if str(r["chat_id"]) == cid_raw), None)
    else:
    ...
    icon = RANK_ICONS.get(rank, f"#{rank}")
    pending_key = f"{user_id}:{rank_info['chat_id']}"

    if action == "usdt":
        # 暂存 USDT 待提交状态，等用户发 /submit_usdt 时按 pending_key 匹配
        _merch_pending[pending_key] = {"rank_info": rank_info, "step": "waiting_usdt", "is_usdt": True}
        await query.edit_message_text(
            f"💰 <b>领取 USDT</b>\n\n"
        ...
        _merch_pending[pending_key] = {"rank_info": rank_info, "step": "waiting_address"}
        await query.edit_message_text(
            f"🎒 <b>币安品牌双肩包 — 填写收货信息</b>\n\n"
            f"群组：{rank_info['chat_title']} {icon} 第 {rank} 名\n\n"
            f"━━━━━━━━━━━━━━━━\n"
...
async def _handle_merch_conversation(update, context) -> bool:
    """拦截私聊文字消息，处理周边实物地址收集对话。
    支持同一用户同时有多个群的待提交状态（_merch_pending key 为 user_id:chat_id）。
    返回 True 表示已消费。"""
    user = update.effective_user
    chat = update.effective_chat
    if not user or not chat or chat.type != "private":
    ...
    # 找出该用户所有进行中的对话状态（排除 waiting_usdt，那个不是文字对话）
    active_keys = [
        k for k, v in _merch_pending.items()
        if k.startswith(f"{user_id}:") and v.get("step") != "waiting_usdt"
    ]
    if not active_keys:
        return False

    # 若有多个进行中，取第一个（先进先出）
    pending_key = active_keys[0]
    state = _merch_pending[pending_key]

    text = update.message.text.strip()
    rank_info = state["rank_info"]
    step = state["step"]
    is_lucky = state.get("is_lucky", False)
    icon = "🎁" if is_lucky else RANK_ICONS.get(rank_info["rank"], f"#{rank_info['rank']}")
    rank_label = "幸运社区" if is_lucky else f"第 {rank_info['rank']} 名"

    if step == "waiting_address":
        state["merch_address"] = text
        state["step"] = "waiting_name"
        # 如果同时有多个群待填写，提示用户当前在处理哪个群
        group_hint = f"（{rank_info['chat_title']}）" if len(active_keys) > 1 else ""
        await update.message.reply_text(
        ...
        del _merch_pending[pending_key]
        # 检查是否还有其他群待填写
        remaining = [k for k in _merch_pending if k.startswith(f"{user_id}:") and _merch_pending[k].get("step") != "waiting_usdt"]
        next_hint = ""
        if remaining:
            next_rank_info = _merch_pending[remaining[0]]["rank_info"]
            next_hint = f"\n\n📬 你还有 <b>{next_rank_info['chat_title']}</b> 的收货信息待填写，请继续回复地址："
        await update.message.reply_text(
            f"✅ <b>收货信息提交成功！</b>\n\n"
            f"群组：{rank_info['chat_title']} {icon} {rank_label}\n"
            f"地址：{state['merch_address']}\n"
            f"姓名：{state['merch_name']}\n"
            f"电话：{state['merch_phone']}\n\n"
        ...
        logger.info(f"[award] merch submitted: uid={user_id} chat={rank_info['chat_id']} rank={rank_info['rank']} info={full_merch}")
        return True
    ...
    for lucky in AWARD_LUCKY:
        try:
            ...
            lucky_msg = (
                ...
                f"奖励：🎒 <b>币安品牌双肩包 ×1</b>\n\n"
                f"━━━━━━━━━━━━━━━━\n"
                f"👇 <b>领奖方式</b>：点击下方链接私信机器人，发送 /start 即可填写收货信息\n"
                f"➡️ <a href='https://t.me/stocknews_forbot'>t.me/stocknews_forbot</a>\n"
                f"ℹ️ 请在 <b>7天内</b> 操作，逾期视为放弃 🎉"
            )
            dm_sent = False
            try:
            ...
            if not dm_sent:
                ...
                sent = await bot.send_message(
                    ...
                    text=(f"🎁 {mention} 恭喜！你的群被选为{AWARD_SEASON_LABEL}幸运社区\n"
                          f"奖励：🎒 <b>币安品牌双肩包 ×1</b>\n"
                          f"点击私信机器人发送 /start 填写收货信息 ➡️ <a href='https://t.me/stocknews_forbot'>t.me/stocknews_forbot</a>"),
                    parse_mode="HTML"
                )
...
async def submit_usdt_command(update, context):
    ...
    # 优先从 pending 状态里取（支持多群：找所有 waiting_usdt 状态）
    usdt_keys = [
        k for k, v in _merch_pending.items()
        if k.startswith(f"{user_id}:") and v.get("step") == "waiting_usdt"
    ]
    if usdt_keys:
        ...
        submitted = []
        for pk in usdt_keys:
            ri = _merch_pending[pk]["rank_info"]
            _save_submission(
                user_id=user_id, username=user.username or "",
                full_name=user.full_name or "", chat_id=str(ri["chat_id"]),
                chat_title=ri["chat_title"], rank=ri["rank"],
                reward_usdt=ri["reward_usdt"], address_type="USDT",
                address="", binance_uid=binance_uid
            )
            del _merch_pending[pk]
            submitted.append(ri)
            logger.info(f"[award] USDT: uid={user_id} rank={ri['rank']} chat={ri['chat_id']} buid={binance_uid}")
        lines = "\n".join(
            f"  · {ri['chat_title']} #{ri['rank']}名 — {ri['reward_usdt']} USDT"
            for ri in submitted
        )
        await update.message.reply_text(
            f"✅ <b>提交成功！</b>\n\n"
    ...
    # fallback：没有 pending 状态时，自动查找所有获奖群一起提交
    rank_list = await _find_all_winner_ranks(context.bot, user.id)
    if not rank_list:
    ...
    for rank_info in rank_list:
        ...
        submitted.append(rank_info)
        logger.info(f"[award] USDT: uid={user_id} rank={rank_info['rank']} chat={rank_info['chat_id']} buid={binance_uid}")
    lines = "\n".join(
        f"  · {ri['chat_title']} #{ri['rank']}名 — {ri['reward_usdt']} USDT"
        for ri in submitted
    )
...
async def submit_merch_command(update, context):
    ...
    user_id = str(user.id)
    rank_list = await _find_all_winner_ranks(context.bot, user.id)
    eligible = [r for r in rank_list if r["rank"] > 3]
    if not eligible:
        if rank_list:
        ...
        return
    # 为每个群初始化 pending 状态
    for rank_info in eligible:
        pending_key = f"{user_id}:{rank_info['chat_id']}"
        if pending_key not in _merch_pending:
            _merch_pending[pending_key] = {"rank_info": rank_info, "step": "waiting_address"}
    # 提示第一个群开始填写
    first = eligible[0]
    icon = RANK_ICONS.get(first["rank"], f"#{first['rank']}")
    extra = f"（共 {len(eligible)} 个群获奖，将逐一填写）\n" if len(eligible) > 1 else ""
    await update.message.reply_text(
        f"🎒 <b>币安品牌双肩包 — 填写收货信息</b>\n\n"
        f"{extra}"
        f"群组：{first['chat_title']} {icon} 第 {first['rank']} 名\n\n"
        f"━━━━━━━━━━━━━━━━\n"
        f"第 1/3 步：请直接回复你的 <b>收货地址</b>\n（省/市/区/街道详情）",
        parse_mode="HTML"
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for ticker in tickers:
        ...
        # ⚠️ 必须在 data 验证通过后才记分，防止无效代码刷排行榜积分
        if int(chat_id) < 0:
